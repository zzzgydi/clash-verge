...
async function resolve() {
  ...
  const appPathList = [
    join("macos/Hiddify Clash Desktop.aarch64.app.tar.gz"),
    join("macos/Hiddify Clash Desktop.aarch64.app.tar.gz.sig"),
  ];
  ...
  fs.copyFileSync(join("macos/Hiddify Clash Desktop.app.tar.gz"), appPathList[0]);
  fs.copyFileSync(join("macos/Hiddify Clash Desktop.app.tar.gz.sig"), appPathList[1]);
  ...
  await uploadAssets(release.id, [
    join(`dmg/Hiddify Clash Desktop_${version}_aarch64.dmg`),
    ...appPathList,
  ]);
}
